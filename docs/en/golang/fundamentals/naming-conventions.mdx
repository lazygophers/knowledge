...
# Naming Conventions - Go Naming Rules
...
type UserId int             // 本知识库推荐：Id 而非 ID

var apiURL string           // 非开头，只首字母大写
var xmlContent []byte       // 非开头，只首字母大写
var userId int              // 非开头，只首字母大写（本知识库推荐）
```

<Badge text="常见缩写词" type="info" />
- **开头**：`HTTP`、`XML`、`URL`、`JSON`、`SQL`
- **非开头**：`apiURL`、`xmlContent`、`jsonParser`


## <Badge text="关于 ID 的特殊说明" type="warning" />

<Callout type="warning" title={<Badge text="重要" type="danger" />}>
  <strong>本知识库命名规范与 Go 官方有所不同</strong>
</Callout>
...
| 规范 | ID 命名 | 示例 |
|-----|---------|------|
| **Go 官方** | 全大写 `ID` | `userID`、`GetUserID` |
| **本知识库** | 首字母大写 `Id` | `userId`、`GetUserId` |

### 本知识库选择 `Id` 的原因

<Badge text="理由" type="info" />

1. **自动生成工具一致**
   - Protocol Buffers (protobuf) 生成 `Id` 而非 `ID`
   - Swagger/OpenAPI 生成 `Id` 而非 `ID`
   - 大多数代码生成工具遵循此约定

2. **命名风格一致**
   - `Id` 符合驼峰命名规则，不是特殊例外
   - `userId` 而非 `userID` - 保持视觉一致性
   - 避免"全大写例外"的复杂性
...
| 缩写词 | 开头全大写，非开头首字母大写 | `HTTPServer`, `userId`（本知识库） |
