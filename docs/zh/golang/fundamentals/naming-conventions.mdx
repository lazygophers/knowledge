...
| 时间范围 | `Start/End` | `int64` | `StartTime`, `EndTime` |
| 持续时间 | `Duration` | `time.Duration` | `Timeout`, `Delay` |
| 截止/过期 | `Deadline/ExpiresAt` | `int64` | `Deadline`, `ExpiresAt` |
...
| `Time` | `int64` | `int64`, `uint64`, `int32`, `uint32` | Unix 时间戳 |
| `Duration` | `time.Duration` | `time.Duration` | 持续时间 |

<Badge text="注意" type="danger" /> **不允许 `*time.Time`**，时间戳字段使用整数类型
...
<Badge text="记忆" type="tip" /> **At/Time 用 int64 时间戳，Duration 用 time.Duration**
...
| 时间范围 | `Start/End` | `int64` 时间戳 | `StartTime int64`, `EndTime int64` |
