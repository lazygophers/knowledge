...
| **Status** | 复合状态结构 | 包含多个字段的状态 | `Status{Code, Message}` |
...
<Badge text="记忆" type="tip" /> **State** = 单一值 / **Status** = 复合结构
...
flowchart TD
    ...
    B -->|单一| C[State / bool]
    B -->|复合| D[Status 结构]
    C --> E[Enabled, Active<br/>StateOn/Off]
    D --> F[Status{Code, Message}<br/>HealthStatus, TaskStatus]
