...
详见 [Struct 类型 - 嵌入](/golang/data-types/struct.mdx#匿名字段嵌入)
...
- **[Struct 类型](/golang/data-types/struct.mdx)** - 方法最常见的接收者类型
- **[Interface 类型](/golang/interface/interface-basics.mdx)** - 方法集决定接口实现
- **[Pointer 类型](/golang/data-types/pointer.mdx)** - 指针接收者详解
...
// 计算周长（值接收者，只读操作）
func (p Circle) Circumference() float64 {
...
**解释**：对于只读操作，使用值接收者。Area() 和 Circumference() 都是计算方法，不修改圆的半径，所以使用值接收者。
...
// 存款（指针接收者，需要修改余额）
func (p *BankAccount) Deposit(amount float64) error {
    if amount <= 0 {
...
// 取款（指针接收者，需要修改余额）
func (p *BankAccount) Withdraw(amount float64) error {
    if amount <= 0 {
...
// 查询余额（值接收者，只读操作）
func (p BankAccount) Balance() float64 {
    return p.balance
}
...
func main() {
    ...
    account.Withdraw(2000.00) // 余额不足

    fmt.Printf("\n最终余额: %.2f\n", account.Balance())
}
```

**解释**：Deposit() 和 Withdraw() 需要修改余额，使用指针接收者。Balance() 只读取余额，使用值接收者。
...
// 实现 fmt.Stringer 接口（值接收者是标准库惯例）
func (p Person) String() string {
    return fmt.Sprintf("Person{Id:%d, Name:%s, Age:%d, Email:%s, Location:%s}",
        p.Id, p.Name, p.Age, p.Email, p.Location)
}
...
**解释**：实现 `String() string` 方法来满足 `fmt.Stringer` 接口。按照标准库惯例，使用值接收者。fmt 包会自动调用此方法。
...
**解释**：Push() 和 Pop() 修改栈内容，使用指针接收者。IsEmpty()、Size()、Peek() 只读取状态，使用值接收者。所有修改方法统一使用指针接收者保持一致性。
...
**解释**：实现 `sort.Interface` 的三个方法：`Len()` 返回长度，`Less()` 比较大小，`Swap()` 交换元素。实现后可以使用 `sort.Sort()` 排序。
...
// 回调函数类型
type OnEvict func(key string, value interface{})
...
// 设置缓存（带过期时间）
func (p *Cache) Set(key string, value interface{}, ttl time.Duration) {
    p.mu.Lock()
    defer p.mu.Unlock()
...
// 清理过期项（接受回调函数）
func (p *Cache) Cleanup(onEvict OnEvict) int {
    p.mu.Lock()
    defer p.mu.Unlock()
...
// 方法值作为回调
func (p *Cache) logEviction(key string, value interface{}) {
    fmt.Printf("[缓存过期] 键: %s, 值: %v\n", key, value)
}
...
func main() {
    ...
    cache.Set("user:2", "Bob", 1*time.Second)
    cache.Set("user:3", "Charlie", 3*time.Second)
    ...
    // 使用方法值作为回调清理过期缓存
    fmt.Println("\n清理过期缓存:")
    evicted := cache.Cleanup(cache.logEviction)
    fmt.Printf("清理了 %d 个过期项\n", evicted)

    // 再次获取
    fmt.Println("\n验证清理结果:")
    _, ok1 := cache.Get("user:1")
    _, ok2 := cache.Get("user:3")
    fmt.Printf("user:1 存在: %v\n", ok1)
    fmt.Printf("user:3 存在: %v\n", ok2)
}
```

**解释**：定义 `OnEvict` 函数类型，`Cleanup` 方法接受回调函数。使用方法值 `cache.logEviction` 作为回调，代码清晰简洁。
