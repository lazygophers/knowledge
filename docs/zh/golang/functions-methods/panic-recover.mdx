...
func getUser(id int) (*User, error) {
    // 模拟数据库查询失败
    return nil, fmt.Errorf("db query failed: %w", ErrNotFound)
}
...
func validateUser(id int) error {
    ...
    if user.Age < 18 {
        return fmt.Errorf("user %d is underage: %d", id, user.Age)
    }
    return nil
}
...
| 错误包装 | `fmt.Errorf("%w", err)` | `fmt.Errorf("query failed: %w", err)` |
...
**解释**：使用 `fmt.Errorf` 和 `%w` 动词包装错误，添加文件名等上下文信息，便于定位问题。
...
func handleUser(id int) error {
    ...
    if err != nil {
        return fmt.Errorf("处理用户请求: %w", err)
    }
    fmt.Printf("用户: %+v\n", user)
    return nil
}
...
func errorHandler(next Handler) Handler {
    return func(input string) error {
        ...
        err := next(input)
        if err != nil {
            log.Printf("[ERROR] 处理 '%s' 失败: %v", input, err)
...
[← 返回函数与方法](../functions-methods/) | [Defer 详解](defer/) | [继续：上下文 →](context/)
